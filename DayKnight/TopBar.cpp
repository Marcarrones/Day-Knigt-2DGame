...
void TopBar::init(glm::ivec2 pos, ShaderProgram& shaderProgram)
{
	...
	for (int i = 0; i < 24; ++i) {
		...
		text = new Text[8];
		text[0].init(glm::vec2(float(pos.x + 176), float(pos.y - 36)), shaderProgram, 2, "VIT");
		text[1].init(glm::vec2(float(pos.x + 176), float(pos.y - 20)), shaderProgram, 2, "EXP");
		text[2].init(glm::vec2(float(pos.x + 420), float(pos.y - 36)), shaderProgram, 2, "SCENE");
		text[3].init(glm::vec2(float(pos.x + 420), float(pos.y - 20)), shaderProgram, 2, "01-01");
		text[4].init(glm::vec2(float(pos.x), float(pos.y - 36)), shaderProgram, 2, "HI-");
		text[5].init(glm::vec2(float(pos.x + 50), float(pos.y - 36)), shaderProgram, 2, "000000");
		text[6].init(glm::vec2(float(pos.x), float(pos.y - 20)), shaderProgram, 2, "1P-");
		text[7].init(glm::vec2(float(pos.x + 50), float(pos.y - 20)), shaderProgram, 2, "000000");

	}
}

void TopBar::restart() {
	text[0].init(glm::vec2(float(60 + 176), float(70 - 36)), shader, 2,  "VIT");
	text[1].init(glm::vec2(float(60 + 176), float(70 - 20)), shader, 2, "EXP");
	text[2].init(glm::vec2(float(60 + 420), float(70 - 36)), shader, 2, "SCENE");
	text[3].init(glm::vec2(float(60 + 420), float(70 - 20)), shader, 2, "01-01");
	text[6].init(glm::vec2(float(60), float(70 - 20)), shader, 2, "1P-");
	text[7].init(glm::vec2(float(60 + 50), float(70 - 20)), shader, 2, "000000");
	for (int i = 0; i < 24; ++i) {
		if (i == 0 || i == 12) spriteVitExp[i].changeAnimation(INICIO);
		else if (i == 23 || i == 11) spriteVitExp[i].changeAnimation(FIN);
		else if (i < 12) spriteVitExp[i].changeAnimation(VIT8);
...
void TopBar::update(int deltaTime)
{
...
void TopBar::calculateVitExp(int vitExp, int option) {
...
void TopBar::setScreen(int scene, int screen) {
	string num = "0" + to_string(scene) + "-0" + to_string(screen);
	text[3].setText(num);
}

void TopBar::setPoints(int points) {
