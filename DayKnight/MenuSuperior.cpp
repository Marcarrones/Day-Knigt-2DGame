...
void MenuSuperior::init(glm::ivec2 pos, ShaderProgram& shaderProgram)
{
	...
	text = new Text[6];
	text[0].init(glm::vec2(float(pos.x + 16), float(pos.y - 32)), shaderProgram, 1,  "X");
	text[1].init(glm::vec2(float(pos.x + 36), float(pos.y - 32)), shaderProgram, 1, "03");
	text[2].init(glm::vec2(float(pos.x + 90), float(pos.y - 32)), shaderProgram, 1, "000000");
	text[3].init(glm::vec2(float(pos.x + 230), float(pos.y - 32)), shaderProgram, 1, "000000");
	text[4].init(glm::vec2(float(pos.x + 400), float(pos.y - 32)), shaderProgram, 1, "STAGE");
	text[5].init(glm::vec2(float(pos.x + 520), float(pos.y - 32)), shaderProgram, 1, "01");
}
